...
describe("Queue", () => {
  ...
  describe("removeFromQueue", () => {
    it("should remove an item from the queue", () => {
      ...
      queue.removeFromQueue("testId");
      expect(queue.queue).not.toContain(newItem);
    });
  });
  describe("clearQueue", () => {
  ...
  describe("Recovery strategy", () => {
    ...
    it("should use custom recovery strategy", async () => {
      const customRecoveryStrategy = jest.fn(
        ({ retry, skip }: RecoveryStrategyProps<any>) => {
          skip();
        }
      );
  ...
  //   describe("dispose", () => {
  //     it("should clean up resources and dispose event listeners", () => {
  //       const queue = createQueue(queueProps);
  //       const pubSubSpy = jest.spyOn(createPubSub.prototype, "dispose");
